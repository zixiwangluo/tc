$(() => {
    ...
    function upload(files) {
        ...
        for (let i = 0; i < files.length; i++) {
			...
			if(type =='imgur'|type =='muke'|type =='gtimg'|type =='catbox'|type =='niupic'|type =='ai58'|type =='imgbox'|type =='sougou'|type =='bilibili'|type =='baidu'|type =='huluxia'){
				var api = 'https://www.hualigs.cn/api/upload?token=24b61055c2e899dd6879f9675bb47614&apiType='+$('#type').val();
				var maxSize = 10485760;
                formData.append('image', files[i]);
			}else if(type =='imgbb'){
				var api = 'https://api.imgbb.com/1/upload?key=08f35721b0311f4f86d6b97cd564fcd0&type='+$('#type').val();
				var maxSize = 33554432;
                formData.append('image', files[i]); 
			}else if(type =='ipfs'){
				var api='https://ipfsapi.glitch.me//api/v0/add?pin=true';
				var maxSize = 104857600;
                formData.append('file', files[i]);
			}else if(type =='baidu2'|type =='douyin'|type =='ks'|type =='lenovo'|type =='mt'|type =='suning'|type =='weidian'){
				var api = 'https://pic.onji.cn/api/'+$('#type').val()+'.php';
				var maxSize = 5242880;
                formData.append('image', files[i]);
				...
				var api = 'https://image.kieng.cn/upload.html?key=9BC634D6-416A-CAAB-790D-23B457BE66C9&type='+$('#type').val();
				var maxSize = 5242880;
                formData.append('image', files[i]);
            }
            ...
            $.ajax({
                ...
                success: res => {
                    if(type =='imgur'){
						...
						var imgSrc = res.data.url.niupic
					}else if(type =='ai58'){
						var imgSrc = res.data.url.ai58
					}else if(type =='imgbox'){
						var imgSrc = res.data.url.imgbox
					}else if(type =='sougou'){
						var imgSrc = res.data.url.sougou
                    }else if(type =='bilibili'){                   
						var imgSrc = res.data.url.bilibili
					}else if(type =='baidu'){ 
						var imgSrc = res.data.url.baidu
					}else if(type =='huluxia'){ 
						var imgSrc = res.data.url.huluxia
					}else if(type =='ttttt'){ 
						var imgSrc = res.data.url.tttt
					}else if(type =='ipfs'){
                    ...
                    if (res.code === -1) {
                        ...
                        if (res.code === 200||res.status === 200||res.Hash) {
                            $('.' + randomClass)
                                .find('.progress-inner')
                                .addClass('success');
                            $('.' + randomClass)
